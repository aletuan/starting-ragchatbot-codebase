...
class TestRAGSystemIntegration:
    ...
    def test_content_specific_query_triggers_search(
        self, mock_anthropic, rag_system_with_data
    ):
        ...
        mock_final_response.content = [
            Mock(
                text="Neural networks are computational models inspired by biological neural networks. They process information through interconnected nodes and are fundamental to deep learning systems."
            )
        ]
    ...
    @patch("ai_generator.anthropic")
    def test_course_outline_query_triggers_outline_tool(
        self, mock_anthropic, rag_system_with_data
    ):
        ...
        mock_final_response.content = [
            Mock(
                text="The Test AI Fundamentals course covers: Introduction (Lesson 0), Neural Networks (Lesson 1), and Deep Learning (Lesson 2)."
            )
        ]
        ...
        response, sources = rag_system_with_data.query(
            "What lessons are in the AI Fundamentals course?"
        )
    ...
    def test_course_filtering_query(self, mock_anthropic, rag_system_with_data):
        ...
        response, sources = rag_system_with_data.query(
            "Tell me about deep learning in the AI Fundamentals course"
        )
    ...
    def test_lesson_specific_query(self, mock_anthropic, rag_system_with_data):
        ...
        mock_final_response.content = [
            Mock(
                text="The introduction covers the basics of artificial intelligence and its applications."
            )
        ]
    ...
    def test_conversation_history_integration(
        ...
        mock_response.content = [
            Mock(
                text="Based on our previous discussion about neural networks, here's more detail."
            )
        ]
        mock_client.messages.create.return_value = mock_response
        mock_anthropic.Anthropic.return_value = mock_client
        ...
        response, sources = rag_system_with_data.query(
            "Tell me more about that", session_id=session_id
        )
    ...
    def test_session_isolation(self, mock_anthropic, rag_system_with_data):
        ...
        history_1 = rag_system_with_data.session_manager.get_conversation_history(
            "session_1"
        )
        history_2 = rag_system_with_data.session_manager.get_conversation_history(
            "session_2"
        )
    ...
    def test_empty_search_results_handling(self, mock_anthropic, rag_system_with_data):
        ...
        mock_final_response.content = [
            Mock(
                text="I don't have specific information about quantum computing in the available courses."
            )
        ]
    ...
    @patch("ai_generator.anthropic")
    def test_course_not_found_error_handling(
        self, mock_anthropic, rag_system_with_data
    ):
        ...
        mock_final_response = Mock()
        mock_final_response.content = [
            Mock(text="I couldn't find a course matching 'Nonexistent Course'.")
        ]
        ...
        response, sources = rag_system_with_data.query(
            "Tell me the basics from the Nonexistent Course"
        )
    ...
    def test_source_attribution_with_links(self, mock_anthropic, rag_system_with_data):
        ...
        mock_final_response = Mock()
        mock_final_response.content = [
            Mock(text="Neural networks information from the course.")
        ]
    ...
    @patch("ai_generator.anthropic")
    def test_multiple_consecutive_queries_same_session(
        self, mock_anthropic, rag_system_with_data
    ):
